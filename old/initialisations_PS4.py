...
#####################################################################
#                                                                   #
#          INITIALISATION ET FONCTIONS BASE ROULANTE                #
#                                                                   #
#####################################################################
...
import Adafruit_PCA9685, time, pygame, sys #, dcEncodeur, queue
...
pygame.init()
#on peut les initialiser séparément
...
def init_servos():
    global pd_init, pg_init
    servo.set_pwm(SERVO_PISTON_D,0,pd_init)
    servo.set_pwm(SERVO_PISTON_G,0,pg_init)
...
#initialiser tous les ESC et les servos
initialiser_esc()
print("Les ESC sont initialisés")
print("")
init_servos()
print("Les servos sont initialisés")
print("")
...
if nb_joysticks > 0:
    ...
    mon_joystick.init()
    print("Le joystick est connecté")
    print("")
    print("La commande est opérationnelle")
    print("")
    print("Presser le bouton 'PS4' pour quitter le programme")
else:
    print("Le joystick n'est pas connecté")

#### Fonctions toggle pistons #####
...
##posBase =  queue.Queue() # queue accueillant les positions cibles
##
############ ATTENTION dans la ligne ci-dessous, il manque la variable port qu'il faudra surement set à :
############ '/dev/ttyUSB1'
##
##baseBras = dcEncodeur.DcEncodeur(servo,ESC_BRAS,pos_q=posBase,vit=25) # initialistation à vitesse  25
##baseBras.start()
    ...
    if len(L) == n:
        ...
        if not wait :            
            for i in range(n) :
                ...
                #print("moteur {} : speed = {} et goal pos = {}".format(i,v,L[i]))
        else:
            for i in range(1,n):
...
baseMilieu = [0,102]  # position de repos basse
baseHaut = [0,0] # position verticale - inutile ?
